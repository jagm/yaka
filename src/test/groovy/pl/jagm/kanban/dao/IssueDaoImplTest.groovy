...
public class IssueDaoImplTest extends Specification {
    ...
    def "test list"() {
        ...
        then:
        1 * sessionFactory.getCurrentSession() >> session
        1 * session.createQuery("SELECT issue FROM Issue AS issue JOIN issue.states AS issueState JOIN issueState.state AS state WHERE state.board = 19 AND issue.deleted = false AND issueState.created = (SELECT MAX(issueState2.created) FROM IssueState AS issueState2 WHERE issueState2.issue = issue.id) ORDER BY importance") >> query
